...
{
    /*
    auto binance = std::make_shared<OrderBook>("Binance", 0.001, 2); // 0.1% taker fee, 0.001 min order size
    auto kucoin = std::make_shared<OrderBook>("KuCoin", 0.0005, 3);  // 0.05% taker fee, 0.01 min order size
    auto okx = std::make_shared<OrderBook>("OKX", 0.0002, 5);       // 0.02% taker fee, 0.001 min order size
    auto uniswap = std::make_shared<OrderBook>("Uniswap", 0.003, 3);  // 0.3% taker fee, 0.1 min order size
    */
    ...
    fs::path data_dir = fs::path(__FILE__).parent_path().parent_path() / "data/knapsack_test";
